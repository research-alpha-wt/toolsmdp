...
### Step 2.1: Validate pipeline on GPU (smoke test)

- Set up Lightning.ai Studio (or Colab)
- `pip install -e ".[train,dev]"` + `pytest tests/ -v`
- Run `scripts/test_base_model.py --mode code_gen` — verify Qwen3.5-4B generates
  ```python blocks at >30% rate
- Run `scripts/test_base_model.py --mode pipeline` — verify full invoke→assimilate→synthesize
  loop with real Pyserini search
- Check: does the model write `<context>` blocks? If not, test with stronger system prompt.
- Run `scripts/test_base_model.py --mode eval` on 50-sample files

**Deliverables:**
- [ ] Environment working on GPU platform
- [ ] Code generation rate measured and >30%
- [ ] `<context>` block generation observed (even if inconsistent at base model stage)
- [ ] Full pipeline validated with real search results

---

### Step 2.2: Run base model WITH tools on eval sets

**Script:** `analysis/pre_training_characterization.py` (new)
...
- Output: `$DATA_ROOT/analysis/rollout_stats/{dataset}_rollout_stats.json`

**Deliverables:**
- [ ] Rollout stats for all eval datasets
